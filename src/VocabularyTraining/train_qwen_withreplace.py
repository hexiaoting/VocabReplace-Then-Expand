...
def instantiate_model_by_mean(
    ...
    replaced_data = pkl.load(open(replaces_pickle,'rb'))
    for tup in replaced_data:
        source_ids = tup[-1]
        source_ids = torch.tensor(source_ids)#, device=device)
        target_embeddings[tup[0]] = source_embeddings[source_ids].mean(dim=0)
        if not tie_word_embeddings:
            target_head_embeddings[tup[0]] = source_head_embeddings[source_ids].mean(dim=0)
...
def main(args, training_args):
    ...
    if args.freeze_model:
        ...
        model,tokenizer = instantiate_model_by_mean(model, source_tokenizer, tokenizer, args.tokenizer_name_or_path+'replaced_tokens.pkl',False)
